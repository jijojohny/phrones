...
export function OperatorConsole() {
  ...
  if (!status) return <main><p className="muted">Loading operator console…</p></main>;

  return (
    <main>
      <header style={{ marginBottom: "1.5rem" }}>
        <span className="badge">Operator</span>
        <h1 style={{ margin: "0.5rem 0 0.25rem", fontSize: "1.5rem" }}>Fund control center</h1>
        <p className="muted" style={{ margin: 0 }}>Monitor fund health, authorize beta testers, review market feed</p>
      </header>

      <section className="card">
        <h2 className="section-title">Operator auth</h2>
        <p className="muted" style={{ fontSize: "0.85rem" }}>Set <code>OPERATOR_API_SECRET</code> in .env for production. Dev mode allows without secret.</p>
        <input
          type="password"
          ...
          {status.preflight.checks.map((c) => (
            <li key={c.label} className={`step ${c.ok ? "done" : ""}`}>
              ...
              <div>
                ...
                <p className="muted">{c.detail}{c.required ? " · required" : " · optional"}</p>
              </div>
            </li>
          ))}
        </ul>
      ...
      <section className="card">
        ...
        {status.betaRequests.length === 0 ? (
          ...
          <table>
            <thead><tr><th>Wallet</th><th>Email</th><th>When</th><th></th></tr></thead>
            <tbody>
      ...
      <section className="card">
        ...
        <table>
          <thead><tr><th>Market</th><th>p_mkt</th><th>p_sent</th><th>Δ</th></tr></thead>
          <tbody>
      ...
      <section className="card">
        ...
        <table>
          <thead><tr><th>Protocol</th><th>Chain</th><th>Share price</th><th>Util</th></tr></thead>
          <tbody>
            {status.vaults.map((v) => (
      ...
      <footer className="page-footer">
        Updated {new Date(status.ts).toLocaleString()} · fund {status.fund.address ? shortenAddress(status.fund.address) : "not deployed"}
      </footer>
    </main>
  );
}
