...
export async function checkAuthorized(fundAddress: string, investor: string): Promise<boolean> {
  const provider = getProvider();
  if (!provider) return false;
  const browser = new BrowserProvider(provider);
  const fund = new Contract(fundAddress, FUND_ABI, browser);
...
export async function depositErc20(
  ...
  if (!allowed) {
    throw new Error("Token not allowlisted on fund — contact ops to enable USDC/USDT");
  }
...
export async function readTokenBalance(
  ...
  const browser = new BrowserProvider(provider);
  const token = new Contract(tokenAddress, ERC20_ABI, browser);
